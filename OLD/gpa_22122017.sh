...
{
echo "     .--------------------------------------------------------------------------------------------------------------------------------."
printf "%-50s %-10s %-10s %-15s %-10s %-12s %-8s %-10s \n" "     |   Nombre" "Estado" "Usuario" "Uptime" "PID" "Fecha" "%CPU" "%Mem        |";
echo "     .-------------------------------------------- ---------- ---------- --------------- ---------- ------------ ----- ---------------."
cont=1
        ...
        i=`cat ./CFG/procesos.lst|sed -n ${cont}p|awk '{print $5}' FS=";"`
				...
				exit 101
							...
							fechaproceso=`${OS_PS} | grep -wi "${pidproceso}" |grep -v grep|grep -v sleep | awk '{print $5,$6}'|sed -n 1p`
                                                        ...
                                                        usu_proceso=`${OS_PS} |grep -w ${pidproceso} |grep -v grep | grep -v sleep| awk '{print $1}' |sed -n 1p`
							usu_proceso_indicado=`cat ./CFG/procesos.lst|sed -n ${cont}p|awk '{print $2}' FS=";"`
							if [ "$usu_proceso" = "$usu_proceso_indicado" ]
								then
									printf "%-51s %-20s %-10s %-15s %-10s %-12s %-5s %-5s \n" "         $apodo_proceso" "$OK" "$usu_proceso" "$tiempoproceso" "$pidproceso" "$fechaproceso" "$porce_cpu" "$porce_mem"
								else
									printf "%-51s %-20s %-21s %-15s %-10s %-12s %-5s %-5s \n" "         $apodo_proceso" "$OK" "${RED}$usu_proceso${REDOF}" "$tiempoproceso" "$pidproceso" "$fechaproceso" "$porce_cpu" "$porce_mem"
							fi
